...
import { useTasks, useDeleteTask, useToggleSchedule } from '@/hooks/queries/useTasks'
import { useQueryClient } from '@tanstack/react-query'
import type { Task, CalendarDayEntry } from '@/types/api'

interface LiveClockProps {
  timezone: string
}

function LiveClock({ timezone }: LiveClockProps) {
...
type EntryType = 'memory' | 'task' | 'scheduled'
...
export default function CalendarView() {
  ...
  async function handleDisableSchedule(id: string): Promise<void> {
    await toggleSchedule.mutateAsync({ id, enabled: false })
    queryClient.invalidateQueries({ queryKey: ['calendar'] })
  }
  ...
  const selectedData: CalendarDayEntry | null = selected ? data[selected] ?? null : null
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      {selected && (() => {
        ...
        const dotColor: Record<EntryType, string> = { memory: 'bg-blue-400', task: 'bg-green-400', scheduled: 'bg-orange-400' }
        const textColor: Record<EntryType, string> = { memory: 'text-muted-foreground', task: 'text-muted-foreground', scheduled: 'text-orange-400' }
        const Icon: Record<EntryType, React.ElementType> = { memory: FileText, task: CheckCircle, scheduled: CalendarClock }
